...
namespace XWUI
{
    ...
    public class ControllableGridLayoutGroup : GridLayoutGroup
    {
        ...
        public int GetCellCountX()
        {
            int cols;
            if (m_Constraint == Constraint.FixedColumnCount) {
                cols = m_ConstraintCount;
            } else {
                float width = rectTransform.rect.width;
                cols = Mathf.Max(1, Mathf.FloorToInt((width - padding.horizontal + spacing.x + 0.001f) / (cellSize.x + spacing.x)));
            }
            return cols;
        }
        ...
        public int GetCellCountY()
        {
            ...
            if (m_Constraint == Constraint.FixedRowCount) {
                ...
                rows = Mathf.Max(1, Mathf.FloorToInt((height - padding.vertical + spacing.y + 0.001f) / (cellSize.y + spacing.y)));
            }
            return rows;
        }
        #endregion
