...
namespace XNodeEditor {
    ...
    public partial class NodeEditorWindow : EditorWindow {
        ...
        public static NodeEditorWindow Open(XNode.NodeGraph graph) {
            ...
            NodeEditorWindow w = GetWindow(typeof(NodeEditorWindow), false, "节点编辑", true) as NodeEditorWindow;
            w.wantsMouseMove = true;
            w.graph = graph;

            w.ValidateGraphEditor();
            if (!w.isReopen)
            {
                w.isReopen = true;
                w.graphEditor.OnWindowCreate();
            }
            return w;
        }

        public static void ChangeSelection(XNode.NodeGraph graph)
        {
            current.graph = graph;
        }
